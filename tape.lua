...
-- Needed Checks
if not fs.exists("/bin/tar.lua") and not fs.exists("/usr/bin/tar.lua") then
  io.stdout:write("Tar is not installed, would you like to install it? (y/n): ")
  local user = io.read()
  if user == "y" then
    print("installing tar...")
    shell.execute("wget https://raw.githubusercontent.com/mpmxyz/ocprograms/master/home/bin/tar.lua /bin/tar.lua")
...
if ops.c == nil and ops.x == nil then
  handleError("no options provided")
end
